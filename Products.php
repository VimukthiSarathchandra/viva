                            ...
                            <div class="row g-1 text-center">
                                ...
                                <div class="nav flex-column nav-pills me-3 mt-2 " role="tablist" aria-orientation="vertical">
                                    <nav class="nav flex-column">
                                        ...
                                        <div class="mt-3 ">
                                            <a href="Products.php" class="text-decoration-none nav-link active">
                        ...
                        <div class="col-12 col-lg-6 mt-3">
                            <div class="row">
                                <div class="offset-lg-2 col-8 col-lg-7 mb-3 mt-4">
                                    <input type="text" class="form-control" placeholder="Search ..." id="admin_search" />
                                </div>
                                <div class="col-3 col-lg-3 d-grid mb-3 mt-4">
                                    <button class="btn btn-primary" onclick="adminSearch();">Search</button>
                                </div>
                            </div>
                        </div>
                            ...
                            <table class="table card-body mt-4">
                                <thead>
                                    <tr class=" border border-1">
                                        ...
                                        <th class="fs-4 d-none d-lg-block">Author</th>
                                ...
                                $b_rs = Database::search("SELECT * FROM `book` INNER JOIN 
                                `author` ON author.id_num=book.author_id");
                                $b_num = $b_rs->num_rows;

                                for ($y = 0; $y < $b_num; $y++) {
                                    $book_data = $b_rs->fetch_assoc();

                                ?>

                                    <tbody>
                                        <tr style="height: 55px;">
                                            <td class=" fs-6 mt-2"><?php echo $book_data["id"]; ?></td>
                                            <td>
                                                <span class="fs-4 p-2"><?php echo $book_data["title"]; ?></span>
                                            </td>
                                            <td class="fw-bold fs-6 pt-3 d-none d-lg-block"><?php echo $book_data["author"]; ?></td>
                                            <td class="fw-bold fs-6  pt-3"><?php echo $book_data["datetime_added"]; ?></td>
                                            <td class="fw-bold fs-6  pt-3">
                                                <?php
                                                if ($book_data["status_id"] == 1) {
                                                ?>
                                                    <button id="pb<?php echo $book_data['id']; ?>" class="btn btn-danger border-0" onclick="blockProduct('<?php echo $book_data['id']; ?>');">Block</button>
                                                <?php
                                                } else {
                                                ?>
                                                    <button id="pb<?php echo $book_data['id']; ?>" class="btn btn-success border-0" onclick="blockProduct('<?php echo $book_data['id']; ?>');">Unblock</button>
                                                <?php
                                                }
                                                ?>
                                            </td>
